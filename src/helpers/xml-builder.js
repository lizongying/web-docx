...
const buildTableProperties = (attributes) => {
  ...
  const tableWordWrap = fragment({ namespaceAlias: { w: namespaces.w } })
    .ele('@w', 'wordWrap')
    .att('@w', 'val', '0')
    .up();
  tablePropertiesFragment.import(tableWordWrap);

  const tableNoWrap = fragment({ namespaceAlias: { w: namespaces.w } })
    .ele('@w', 'noWrap')
    .up();
  tablePropertiesFragment.import(tableNoWrap);
...
const buildTable = async (vNode, attributes, docxDocumentInstance) => {
  ...
  if (isVNode(vNode) && vNode.properties) {
    ...
    if (width) {
      ...
      modifiedAttributes.width = attributes.maximumWidth;
    }
...
const buildImage = async (docxDocumentInstance, vNode, maximumWidth = null) => {
  ...
  if (response) {
    ...
    const imageFragment = await buildParagraph(
      vNode,
      {
        type: 'picture',
